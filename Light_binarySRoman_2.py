...
Tobs =float(62.0)
RomanAAC=float(0.1*1000.0)## mili arcsec 
...
def prior(p, prt): 
    ...
    if(u01>sm and u01<1.0 and t01>0.0 and t01<Tobs and tE1>0.0 and tE1<100.0 and fb1>sm and fb1<1.0 and rho1>sm and rho1<10.0
...
def mcmcp(prt, tim, Magni, erra, NUM, u0FF, t0FF, tEFF):
    ...
    p0[:,3]=np.abs(np.random.normal(prt[3],0.2,   Nwal))#fb
...
def funcini(tim , Atot, tE):
    ...
    #################################
    ### tE_fitted ####
    tEr=np.arange(float(tE-tE*0.9), float(tE+tE*0.9),  0.001)
...
save2=open("./files/BestfitR2.dat","a+")
save2.close()
f1=open("./files/resGBRomanA.dat","r")
nf= sum(1 for line in f1)  
par=np.zeros((nf,nc))
detec=np.zeros((nf,nc))
par2=np.zeros((nf*5,10))
par=np.loadtxt("./files/resGBRomanA.dat")
par2=np.loadtxt("./files/resGBRomanB.dat")  
...
for i in range(nf):
    ...
    chi2, chiR, chiCM, fb, Magb, threef  =  par[i,46], par[i,47], par[i,48], par[i,49], int(par[i,50]), par[i,51]

    Dmag=abs(2.5*np.log10(f21))
    sproj= float(Semi*Rsun*np.sin(inc*np.pi/180.0)/AU/Ds)##mas
    cri2=sproj/RomanAAC## unresolvable
    prt=np.array([u0, t0, tE, fb, 0.5*(ros1+ros2)])          
    ############################################################################
    if(dchiL>=300.0 and peak>0 and threef>0 and ei==621):
        ...
        asym=Asymetry(mod[:,9])/np.mean(np.abs(dat[:,2]))#normalized 
        #######################################################################
        ...
        plt.plot(mod[:,0],mod[:,9],'k-', label=r"$\rm{Binary}~\rm{System}$",lw=1.8)
        if(flag>0): plt.plot(timt,mfit,'g-.', label=r"$\rm{ESPL}~\rm{Fitted}~\rm{model}$",lw=1.85)
        plt.errorbar(dat[:,0],dat[:,1],yerr=dat[:,2],fmt=".",markersize=3.7,color='m',ecolor='pink',elinewidth=0.07,capsize=0,alpha=0.2)
        ...
        r"$,~\mathcal{A}_{\rm{n}}=$"+str(round(asym,1))+"\n"+
        r"$t_{\rm{E},b}\rm{(days)}=$"+str(round(tEf,1))+r"$\pm$"+str(round(dtE,2))+
        ...
        ax1.legend(prop={"size":13.0}, loc=1)
        ...
        fig=plt.gcf()
        fig.tight_layout()
        fig.savefig("./FigsRoman/LBinaryR{0:d}_{1:d}.jpg".format(int(ei),int(flag)), dpi=200)
        #######################################################################
        print("************ Light curve was plotted ***********************")    



print("faction of simplw events with Dm<2 && Sp<RomanA:", float(simple*100.0/nlens)) 
print("BS Microlensing events with Delta m<2:  ",         float(frac1*100.0/nlens))    
print("BS Microlensing events with sp<Roman_Accuracy:  ", float(frac2*100.0/nlens))    
...
for i in range(nc):
    ...
    fig=plt.gcf()
    fig.savefig("./FigsRoman/Histo/histR{0:d}.jpg".format(i),dpi=200)
...
for j in range(5): 
    for i in range(10):
        ...
        fig= plt.figure(figsize=(8,6))
        ax= plt.gca()              
        plt.hist(par2[j:int(nf*5):5,i],40,histtype='bar',ec='k',facecolor='green',alpha=0.5,rwidth=1.5,label=r"$\rm{Sim}~\rm{events}$")
        y_vals = ax.get_yticks()
        ax.set_yticks(y_vals)
        ax.set_yticklabels(['{:.2f}'.format(float(1.0*x*(1.0/nf))) for x in y_vals]) 
        y_vals = ax.get_yticks()
        ...
        plt.grid(linestyle='dashed')
        fig=plt.gcf()
        fig.savefig("./FigsRoman/Histo/histRB{0:d}_{1:d}.jpg".format(i,j),dpi=200)
print ("****  All histos(2) are plotted ***********************  ")   
################################################################################
