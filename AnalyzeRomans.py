...
nam=[r"$\rm{Nsimulation}$", r"$\rm{Ndetection}$",r"$\rm{Galactic}~\rm{longitude}$", r"$\rm{Galactic}~\rm{Latitude}$",
     ...
     r"$\chi^{2}_{\star,1}$", r"$\chi^{2}_{\star,2}$",r"$\chi^{2}_{\rm{real}}$",r"$\chi^{2}_{\rm{CM}}$",r"$f_{b}$", r"$m_{\rm{base}}(\rm{mag})$", r"$Flag_{3}$"]
...
detec2=np.zeros((nf*5,10))
par= np.loadtxt("./files/resGBRomanA.dat")  
par2=np.loadtxt("./files/resGBRomanB.dat")  
ana= np.loadtxt("./files/BestfitR.dat")  
nn=int(0)
good=np.zeros((3000,16))
magb=np.zeros((5)); Map1=np.zeros((5)); Map2=np.zeros((5)); 
...
for i in range(nf):
    ...
    chi2, chiR, chiCM, fb, mbase, threef  =  par[i,46], par[i,47], par[i,48], par[i,49], int(par[i,50]), par[i,51]
    for k in range(5): 
        cc=int(i*5+k)
    ...
    ###########################################################################
    if(dchiL>=300.0 and peak>0 and threef>0 ):#and magb[4]>14.8 and magb[4]<26.0 
        detec[int(nlens),:]=par[i,:]
        for k in range(5): 
            detec2[int(nlens*5+k),:]=par2[int(i*5+k),:]
        nlens+=1
        ...
        if(Dmag<2.0 and cri2<=1.0):  
            ...
            if(nn>=0):         
                ...
                if(abs(chi2m-chiR)<100 and abs(tE-tEf)<50.0): 
                    ...
                    good[simple,:]=np.array([tE,fb,u0,ros, Dmag, sproj, mbase, tEf, rosf, fbf, u0f,  etE, eros,ei,2.0*abs(tE-tEf)/tE,q ])#15
                    simple+=1; 
                    if(abs(tE-tEf)>100.0): 
...
fmt="Roman & $%.2f$ & $%.2f$ & $%.2f$ & $%.4f$ & $%.2f$ & $%.2f$ & $%.4f$ & $%.2f$ & $%.2f$ & $%.3f$")    
save2.close()
...
for i in range(7): 
    ...
    fig=plt.gcf()
    fig.tight_layout()
    fig.savefig("./FigsRoman/Histo/DtE{0:d}.jpg".format(i), dpi=200)             
    ###########################################################################                      
    plt.cla()
    plt.clf()
    fig= plt.figure(figsize=(8,6))
    ...
    fig=plt.gcf()
    fig.tight_layout()
    fig.savefig("./FigsRoman/Histo/Dros{0:d}.jpg".format(i), dpi=200) 
...
plt.text(1.25,-1.55,r"$\rm{Real}~\rm{values}$", fontsize=18, color="r", alpha=1.0)
plt.text(1.25,-1.7,r"$\rm{Best}-\rm{fitted}~\rm{values}$", fontsize=18, color="g", alpha=1.0)
plt.title(r"$\rm{Simple}-\rm{like}~\rm{and}~\rm{BS}~\rm{Microlensing},~\rm{Roman}$", fontsize=18, color="k")
plt.xticks(fontsize=18, rotation=0)
plt.yticks(fontsize=18, rotation=0)
plt.xlim([np.log10(1.),     np.log10( 69.0)])
plt.ylim([np.log10(5.0e-5), np.log10(0.038)])
#plt.yscale('log')
#plt.xscale('log')
#plt.grid(True)
#plt.grid(linestyle='dashed')  
fig=plt.gcf()
fig.tight_layout()
fig.savefig("./FigsRoman/Histo/DmapR.jpg", dpi=200)                   
...
for i in range(nc):
    ...
    plt.grid(linestyle='dashed')
    fig=plt.gcf()
    fig.savefig("./FigsRoman/Histo/histL{0:d}.jpg".format(i),dpi=200)
...
for j in range(5): 
    for i in range(10):
        ...
        plt.grid(linestyle='dashed')
        fig=plt.gcf()
        fig.savefig("./FigsRoman/Histo/histLB{0:d}_{1:d}.jpg".format(i,j),dpi=200)
